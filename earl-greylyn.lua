-- earl-greylyn.lua
-- A Neovim 0.12 colorscheme based on the Earl Grey color scheme
-- https://earl-grey.halt.wtf
--
-- Installation: place this file at
--   ~/.config/nvim/colors/earl-greylyn.lua
-- then add to your config:
--   vim.cmd("colorscheme earl-greylyn")

vim.cmd("highlight clear")
if vim.g.syntax_on then vim.cmd("syntax reset") end
vim.g.colors_name = "earl-greylyn"
...
local c = {
  ...
  -- Hue blends (light tints, useful for backgrounds/selections)
  purple2     = "#E4DAE0",
  purple3     = "#D8CAD4",
  blue1       = "#EBECEF",
  blue2       = "#DBDEE5",
...
hi("PmenuSel",         { fg = c.fg,      bg = c.purple2, bold = true })
...
hi("@namespace",               { fg = c.fg,      italic = true })  -- legacy alias
...
-- Headings: each level gets its own distinctive color so you can visually
-- orient yourself in long documents at a glance.

hi("@markup.heading",          { fg = c.purple,  bold = true })     -- fallback
hi("@markup.heading.1",        { fg = c.purple,  bold = true })     -- # H1  — purple
hi("@markup.heading.2",        { fg = c.blue,    bold = true })     -- ## H2  — blue
hi("@markup.heading.3",        { fg = c.teal,    bold = true })     -- ### H3 — teal
hi("@markup.heading.4",        { fg = c.orange,  bold = true })     -- #### H4
hi("@markup.heading.5",        { fg = c.green,   bold = true })     -- ##### H5
hi("@markup.heading.6",        { fg = c.comment, bold = true })     -- ###### H6 — muted

-- Markdown-specific heading marker groups (used by the markdown TS grammar)
...
-- Neovim 0.12 uses scoped heading groups with a filetype suffix
-- (e.g. @markup.heading.1.markdown).  Define both the generic and specific
-- forms so the colorscheme works regardless of TS grammar version.
...
-- The Elixir TS grammar exposes many Elixir-specific capture groups on top of
-- the generic @keyword / @function base groups above.
...
hi("@variable.special.elixir",     { fg = c.purple })   -- self, __MODULE__, etc.
hi("@module.elixir",               { fg = c.fg,      italic = true })
hi("@string.elixir",               { fg = c.green })
hi("@string.sigil.elixir",         { fg = c.orange })   -- ~r//, ~s//, etc.
hi("@string.escape.elixir",        { fg = c.teal })
hi("@number.elixir",               { fg = c.teal })
hi("@constant.elixir",             { fg = c.teal })      -- :atoms
hi("@symbol.elixir",               { fg = c.teal })      -- alias for atoms in some grammars
hi("@attribute.elixir",            { fg = c.orange,  italic = true }) -- @doc, @spec, @moduledoc
hi("@punctuation.special.elixir",  { fg = c.orange })   -- interpolation #{...}
...
hi("BlinkCmpMenuSelection",        { fg = c.fg,     bg = c.purple2, bold = true })
...
-- ── Indent guides (indent-blankline / snacks.nvim indent) ────────────────────
...
-- render-markdown.nvim paints heading backgrounds; use the same hue tints.
...
vim.g.terminal_color_0  = c.grey2     -- black
...
vim.g.terminal_color_9  = "#A86662"   -- bright red
vim.g.terminal_color_10 = "#8A9060"   -- bright green
vim.g.terminal_color_11 = "#A08054"   -- bright yellow
vim.g.terminal_color_12 = "#6B7FAB"   -- bright blue
vim.g.terminal_color_13 = "#9A6D97"   -- bright magenta
vim.g.terminal_color_14 = "#5C9293"   -- bright cyan
